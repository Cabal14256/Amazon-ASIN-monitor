...
如需按 analytics 超时治理方案拆成“公网 API + 内网 scheduler/API + worker”三角色，请参考：

- `docs/1Panel-三角色部署教程.md`
- `server/deploy/1panel/public-api.env.example`
- `server/deploy/1panel/internal-scheduler-api.env.example`
- `server/deploy/1panel/worker.env.example`
...
- API 反向代理：`/api` -> `http://127.0.0.1:3001`
