...
# 批量检查任务并发（默认2）
BATCH_CHECK_GROUP_CONCURRENCY=2
# 前端分析页是否启用Web Worker（构建时读取）
ANALYTICS_WEB_WORKER_ENABLED=false
...
```bash
cd server
npm run start:api      # PROCESS_ROLE=api，仅API+调度
npm run start:worker   # PROCESS_ROLE=worker，仅队列消费者
npm run start:all      # PROCESS_ROLE=all，API+消费者（默认兼容模式）
```
...
1. ⚠️ **时区问题**：系统统一使用北京时间（`Asia/Shanghai`），请在环境变量中设置 `TZ=Asia/Shanghai`
...
**方式 1：在 1Panel 宿主机直接执行**

```bash
chmod +x deploy.sh
sudo ./deploy.sh --archive /root/amazon-asin-monitor-deploy.tar.gz --commit-label <commit>
```

`deploy.sh` 适用于 1Panel 宿主机，默认按当前线上拓扑执行：

1. 可选解压上传的发布包到 `/opt/Amazon-ASIN-monitor`
2. 备份当前前端静态目录、`server/src`、`server/scripts`、相关 `package.json`
3. 将 `/opt/Amazon-ASIN-monitor/dist` 原子切换到 `/opt/1panel/www/sites/Amazon-ASIN-monitor/index`
4. 按 `worker -> scheduler -> api` 顺序重启 1Panel Node 容器
5. 检查 `http://127.0.0.1:3001/health` 和 `https://127.0.0.1/health`
...
# 解压发布包并部署
sudo ./deploy.sh \
  --archive /root/amazon-asin-monitor-deploy.tar.gz \
  --commit-label 7dde213

# 如果 package-lock.json 发生变化，需要在宿主机补装依赖
sudo ./deploy.sh \
  --archive /root/amazon-asin-monitor-deploy.tar.gz \
  --install-server \
  --commit-label 7dde213
...
**方式 2：在本地 Windows 机器打包并通过 SSH/SCP 触发远端部署**

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\deploy-1panel.ps1 `
  -ServerHost 139.224.73.167 `
  -User root
```

本地包装脚本会：

1. 默认先执行本地 `npm run build`
2. 打包仓库（自动排除 `.git`、`node_modules`、`.env` 等不应发布的内容）
3. 默认从 `server/.env` 读取 `DB_PASSWORD` 作为 SSH 密码，也可以用 `-Password` 覆盖
4. 通过本地 `python + paramiko` 上传发布包和最新 `deploy.sh`
5. 通过 SSH 调用远端 `deploy.sh`
