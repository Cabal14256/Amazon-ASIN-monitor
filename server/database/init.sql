...
CREATE TABLE IF NOT EXISTS `asins` (
  ...
  `manual_broken_updated_at` DATETIME DEFAULT NULL COMMENT '人工标记异常更新时间',
  `manual_broken_updated_by` VARCHAR(100) DEFAULT NULL COMMENT '人工标记异常操作人',
  `manual_excluded_from_group` TINYINT(1) DEFAULT 0 COMMENT '排除父变体人工标记: 0-否, 1-是',
  `manual_excluded_reason` VARCHAR(500) DEFAULT NULL COMMENT '排除父变体人工标记原因',
  `manual_excluded_updated_at` DATETIME DEFAULT NULL COMMENT '排除父变体人工标记更新时间',
  `manual_excluded_updated_by` VARCHAR(100) DEFAULT NULL COMMENT '排除父变体人工标记操作人',
  `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `last_check_time` DATETIME DEFAULT NULL COMMENT '监控更新时间（上一次检查的时间）',
  `feishu_notify_enabled` TINYINT(1) DEFAULT 1 COMMENT '飞书通知开关: 0-关闭, 1-开启',
  ...
  INDEX `idx_is_broken` (`is_broken`),
  INDEX `idx_manual_broken` (`manual_broken`),
  INDEX `idx_manual_excluded_from_group` (`manual_excluded_from_group`),
  INDEX `idx_last_check_time` (`last_check_time`),
  INDEX `idx_feishu_notify_enabled` (`feishu_notify_enabled`),
