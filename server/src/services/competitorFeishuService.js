...
function recordRateLimit(country) {
  ...
  if (feishuMetrics.rateLimitCount % 5 === 0) {
    console.warn(
      `[竞品feishu] 累计 ${feishuMetrics.rateLimitCount} 次限频（最近一次: ${country}）`,
    );
  }
}
...
setInterval(() => {
  if (feishuMetrics.rateLimitCount > 0) {
    console.warn(
      `[竞品feishu] 最近 ${RATE_LIMIT_LOG_INTERVAL / 1000}s 触发 ${
        feishuMetrics.rateLimitCount
 ...
 */
async function sendCompetitorFeishuNotification(region, messageData) {
  try {
    // 获取竞品飞书配置（使用区域代码）
    const config = await CompetitorFeishuConfig.findByRegion(region);

    if (!config || !config.webhook_url) {
      const displayCountry =
        messageData.countryDisplay || messageData.country || region;
      console.log(
        `区域 ${region} (${displayCountry}) 未配置竞品飞书Webhook，跳过通知`,
      );
      return { success: false };
    }

    // 构建飞书消息卡片
    const card = buildCompetitorFeishuCard(messageData);
    ...
    if (response.status === 200 && response.data.code === 0) {
      const displayCountry =
        messageData.countryDisplay || messageData.country || region;
      console.log(`✅ 竞品飞书通知发送成功: ${region} (${displayCountry})`);
      return { success: true };
    } else {
      const displayCountry =
        messageData.countryDisplay || messageData.country || region;
      console.error(`❌ 竞品飞书通知发送失败: ${region} (${displayCountry})`, {
    ...
    const displayCountry =
      messageData.countryDisplay || messageData.country || region;
    console.error(
      `❌ 发送竞品飞书通知异常: ${region} (${displayCountry})`,
      error.message,
    );
    return {
      success: false,
...
async function sendCompetitorNotificationWithRetry(region, messageData) {
  let attempts = 0;
  while (attempts < MAX_RETRY_ATTEMPTS) {
    attempts++;
    const result = await sendCompetitorFeishuNotification(region, messageData);
    if (result.success) {
      return result;
    }
 ...
 */
function buildCompetitorFeishuCard(data) {
  const {
    title = '竞品ASIN变体监控通知',
    country,
    totalGroups = 0,
 ...
 */
async function sendCompetitorBatchNotifications(countryResults) {
  const results = {
    total: 0,
    success: 0,
    failed: 0,
  ...
  for (let i = 0; i < countries.length; i += BATCH_SIZE) {
    ...
    const tasks = batch.map(async (country) => {
      ...
      const result = await sendCompetitorNotificationWithRetry(
        region,
        notificationData,
      );
      if (result.success) {
        ...
        if (result.errorCode === RATE_LIMIT_CODE) {
          console.warn(`[竞品feishu] 国家 ${country} 限频重试失败`);
