...
async function getOverallStatistics(params = {}) {
  ...
  if (canUseAggDuration) {
    try {
      durationRows = await getDurationSourceRowsFromAgg({
        startTime,
        endTime,
        sourceGranularity,
        country,
      ...
      logger.warn(
        '[统计查询] getOverallStatistics 聚合表读取失败，回退原始表',
        {
  ...
  if (!Array.isArray(durationRows)) {
    durationRows = await getDurationSourceRowsFromRaw({
      startTime,
      endTime,
      sourceGranularity,
      country,
