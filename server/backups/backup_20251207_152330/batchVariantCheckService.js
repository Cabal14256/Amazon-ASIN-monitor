      ...
      if (response && response.items) {
        ...
        for (const item of response.items) {
          ...
          if (Array.isArray(item.relationships)) {
            item.relationships.forEach((relByMp) => {
              if (!relByMp || !Array.isArray(relByMp.relationships)) return;

              relByMp.relationships.forEach((rel) => {
                if (!rel || rel.type !== 'VARIATION') return;

                // 有 parentAsins：当前 ASIN 是子变体
                if (
                ...
                // 有 childAsins：当前 ASIN 是父变体
                if (
                  Array.isArray(rel.childAsins) &&
                  rel.childAsins.length > 0
                ) {
                  hasVariants = true;
                  if (!parentAsin) {
                    parentAsin = asin; // 自己就是父
                  }
                }
              });
            });
          ...
          // 兼容旧版的 variations 字段（如果 relationships 里没东西）
          if (
            !hasVariants &&
            ...
            hasVariants = true;
            const parentGroup = item.variations.find(
              (g) => g.variationType === 'PARENT',
            );
            if (parentGroup && !parentAsin) {
              parentAsin = asin;
            }
          }
