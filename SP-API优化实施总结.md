...
2. **提高可用性**: HTML 抓取兜底确保即使 SP-API 完全失败也能获取数据
3. **简化配置**: 简化模式无需 AWS 签名，降低配置复杂度
4. **灵活调整**: 所有关键参数都可通过环境变量或前端界面调整

## 注意事项

1. **分批处理与实时性**: 分批处理会影响飞书通知的实时性，如果对实时性要求高，可以设置 `MONITOR_BATCH_COUNT=1` 完全取消分批
2. **HTML 抓取风险**: HTML 抓取可能违反 Amazon 服务条款，建议仅在必要时启用
3. **缓存一致性**: 缓存时间增加到 12 分钟，可能影响数据实时性，但可以显著降低 API 调用频率
4. **版本兼容性**: SP-API 版本策略会自动适配不同版本的响应格式
